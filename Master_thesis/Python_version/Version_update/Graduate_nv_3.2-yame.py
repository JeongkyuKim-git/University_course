...
class Node(object):
    ...
    """
    def __repr__(self):
        look_up_table = "["+(self.data + ", "+ str(self.key) + ", "+ self.sequence)+"]"
...
if __name__ == "__main__":
    ...
    reference_file = open('Cryptomonas_curvata_no_space.fa', 'r')  # file open reference
    """#rf_file_value"""

    query_file = open('Cryptomonas_curvata_no_space.fa', 'r')  # file open reference
    """#query_file_value"""


    #"""
    #reference_file = open('./Cryptomonas curvata.fa', 'r')  # file open reference
    #query_file = open('./Cryptomonas_curvata_1-42761_v1.fa', 'r')  # file open reference
    #"""

    Reference_list = []  # Reference_list = []
    k_mer_reference = []  # k-mer splice_window
    k_mer_query = []  # k-mer splice_window
    common_short_read = [] # common short-read(data)
    ...
    w = window_size()
    result_r = w.k_mer(reference_file.readline(), int(k_mer_user))
    result_q = w.k_mer(query_file.readline(), int(k_mer_user))
    ...
    print(t.leafs)
    common_short_read.append(t.leafs) # common short-read
    print(common_short_read)
    common_read_find = []
    common_read_find = list(itertools.chain(*common_short_read))
    common_exact = []

    print()
    print(common_read_find)
    for i in common_read_find:
        common_exact.append(i)
        print(i)
